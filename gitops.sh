...
# Build fleet args: all fleet files under fleets/
fleet_args=()
...
# Apply global config + all teams (dry-run first to validate).
$FLEETCTL gitops -f "$FLEET_GLOBAL_FILE" "${fleet_args[@]}" "${delete_args[@]}" --dry-run

if [ "$FLEET_DRY_RUN_ONLY" = true ]; then
  exit 0
fi

$FLEETCTL gitops -f "$FLEET_GLOBAL_FILE" "${fleet_args[@]}" "${delete_args[@]}"
